<template>
  <nav class="flex items-center w-full p-2 pb-0">
    <div
      class="flex items-center p-1 mr-2 font-mono transition-colors duration-150 border rounded-md cursor-pointer select-none text-quicksilver hover:text-melon hover:bg-davys border-quicksilver"
      :class="{ 'text-melon': $route.fullPath === '/' }"
    >
      <NuxtLink to="/">{{ $t("CREATE") }}</NuxtLink>
    </div>
    <div
      class="flex items-center p-1 font-mono transition-colors duration-150 border rounded-md cursor-pointer select-none text-quicksilver hover:text-melon hover:bg-davys border-quicksilver"
...
export default {
  ...
  data() {
    return {
      ...
      displayName: this.parseDisplayName(),
    };
  },
};
</script>